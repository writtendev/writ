...
// WRIT-60 spike (5k records x one assignee each). Once downstream bridge/import tooling
...
func seedBenchDB(tb testing.TB, db *projection.DB) {
	...
	recStmt, err := tx.Prepare("INSERT INTO o_record (object_id, f_title, f_status) VALUES (?, ?, ?)")
	if err != nil {
		tb.Fatalf("prepare records: %v", err)
	}
	defer recStmt.Close()

	asStmt, err := tx.Prepare("INSERT INTO o_record__assignees (object_id, item) VALUES (?, ?)")
	if err != nil {
	...
	for i := 0; i < benchNumRecords; i++ {
		...
		if _, err := recStmt.Exec(recID, fmt.Sprintf("Record %d for feature", i), status); err != nil {
			tb.Fatalf("insert record: %v", err)
		}

		if i%4 != 0 {
			// Assigned
			assignee := assignees[i%len(assignees)]
			if _, err := asStmt.Exec(recID, assignee); err != nil {
...
func BenchmarkObjectGet(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		res, err := db.Object("rec-42")
		if err != nil {
			b.Fatalf("Object: %v", err)
		}
		if res.ObjectID != "rec-42" {
			b.Fatalf("expected rec-42, got %s", res.ObjectID)
		}
	}
}
...
func TestQueryPerformanceBudget(t *testing.T) {
	...
	t.Run("ObjectGetBudget", func(t *testing.T) {
		...
		for i := 0; i < iterations; i++ {
			...
			if res.ObjectID != "rec-42" {
				t.Fatalf("expected rec-42, got %s", res.ObjectID)
			}
		}
		avg := time.Since(start) / iterations
		if avg > budget {
