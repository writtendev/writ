...
var allowedImports = map[string]bool{
	...
	`"github.com/writtendev/writ/internal/dag"`:     true,
	`"github.com/writtendev/writ/internal/resolve"`: true,
	`"github.com/writtendev/writ/internal/state"`:   true,
}
	...
	for _, entry := range entries {
		...
		for _, imp := range file.Imports {
			...
			if !allowedImports[pathVal] {
				t.Errorf("%s imports forbidden package %s", filepath.Base(name), pathVal)
			}
		}
	}
}
