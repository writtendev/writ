...
3. **Fold** — the heart. One generic reducer, not one per type: ops plus declarative rules in, materialized state out (`Fold(ops, rules) → ObjectState{state map[string]any}`), concurrent-edit resolution exactly as fixtures dictate (last-writer-wins with op-id tiebreak as the default rule, where "last" is position in the causality-monotone total order; per-field rules where the schema says so, drawn from the closed strategy catalogue in `spec/fold.md`). Pure functions, no I/O, thoroughly testable. Must be boring and correct. Rules drawn from a folded `schema` object are WRIT-186's target, not what ships today: the driver already takes `rules []Rule` generically, but those rules still come from hard-coded `field-rules.json` tables (`spec/fieldrules.go`), not a folded `schema` object.
