...
`Store.vocabularies`' producer-vocabularies cache is fingerprinted over `dag.Chains`, and until WRIT-202, a cache *hit* on the append path still paid one full `dag.Chains` ref walk — every `Append` linear in the repository's total ref count (`BenchmarkAppendByRefCount` at 0/200/500/2,000 loose refs, re-measured for WRIT-202 round 2 on this branch's head — Apple M2 Max, `-benchtime=200x -count=3`, medians of three, "before" being this same tree with the window disabled: 1.16ms → 8.51ms → 20.03ms → 67.98ms per append, against 1.20ms → 1.26ms → 1.40ms → 2.24ms with the window in place, a slope of ~33.1µs per ref falling to ~0.53µs, about 62x). The fix that would obviously restore flat per-append cost — trust the rolled-forward snapshot `Store.noteAppend` already maintains, and stop re-deriving from `dag.Chains` at all — reintroduces a correctness bug: a second `writ.Store` handle on the same repository (a CLI plus a watching client, writ's normal case) never fires the first handle's chain observer, so its snapshot goes stale the moment the other handle applies a schema change, with nothing to notice.
