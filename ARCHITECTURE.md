...
`Store.vocabularies`' producer-vocabularies cache is fingerprinted over `dag.Chains`, and until WRIT-202, a cache *hit* on the append path still paid one full `dag.Chains` ref walk — every `Append` linear in the repository's total ref count (measured at ~0.75ms flat on `main` vs. 0.75ms → 6.7ms → 21ms → 114ms at 0/200/500/2,000 loose refs). The fix that would obviously restore flat per-append cost — trust the rolled-forward snapshot `Store.noteAppend` already maintains, and stop re-deriving from `dag.Chains` at all — reintroduces a correctness bug: a second `writ.Store` handle on the same repository (a CLI plus a watching client, writ's normal case) never fires the first handle's chain observer, so its snapshot goes stale the moment the other handle applies a schema change, with nothing to notice.
...
The accepted risk, stated plainly: within the window, a second handle can sign an op that a schema the first handle just applied would have refused. Those ops are permanent — the reader withholds or quarantines the affected field rather than corrupting anything (`spec/fold.md` §7.1's total reader), so the outcome stays visible, but it does not un-happen. This is accepted, not a defect to close later by widening what the append path observes; the filesystem fast path an earlier round of this same ticket tried, and reverted, is closed for the same reason and is not being reopened by this decision (see the ticket for the two correctness defects that closed it).
