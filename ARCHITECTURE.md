...
3. **Fold** — the heart. One generic reducer, not one per type: ops in plus declarative rules out of a folded `schema` object, materialized state out (`Fold(ops, rules) → ObjectState{state map[string]any}`), concurrent-edit resolution exactly as fixtures dictate (last-writer-wins with op-id tiebreak as the default rule, where "last" is position in the causality-monotone total order; per-field rules where the schema says so, drawn from the closed strategy catalogue in `spec/fold.md`). Pure functions, no I/O, thoroughly testable. Must be boring and correct.
...
store.Objects.Create(ctx, objectType, fields)  // append the creating op for a schema-declared type
store.Objects.Apply(ctx, objectID, op)         // append a further op against an existing object
store.Objects.Get(ctx, objectID)               // folded state as map[string]any, per the schema
store.Query.Objects(filter)                    // query across types, served from the projection
store.Schema()                                 // the folded schema currently in effect
...
The shapes callers see come from the schema folded out of the log, not from Go structs writ ships: `engine/state`'s per-type structs (`Review`, `Comment`, `Issue`, `Anchor`, …) stop being the public shape, and `store.Objects.Get` returns the generic folded map the schema describes instead. Typed codegen — generating a consumer's own Go structs from its schema — is deliberately deferred until a real consumer needs it; the DX regression in the meantime (map access instead of typed fields) is a known, accepted cost, not an oversight.
...
The schema layer makes the corpus schema-parametric (WRIT-190): a fixture now carries its own schema alongside its ops, and asserts "given this schema and these ops, the fold produces X" rather than assuming one hard-coded vocabulary. Be honest about the trade — this is a *wider* conformance surface, not a narrower one, since it now spans the cross-product of value types and merge strategies rather than one fixed set of typed fields.
