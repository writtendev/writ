...
Writ hard-codes exactly one object type: `schema`. That is the bootstrap and the only permitted exception — every other type is declared by a `schema` object written into the log, not baked into writ's spec (see §Schema layer below). Object IDs and cross-references are globally unique (`<repo-id>#<object-id>` or bare `<object-id>` for repo-local references, where IDs are 128-bit random lowercase hex strings; decided and spec'd in `spec/identifiers.md`, WRIT-16), with the qualified form an opaque pointer to an object that may live in another repo, so an object in repo A referring to an object in repo B is representable regardless of which layer defines either type — the one-graph query is the point. Every object, whatever its schema-declared type, homes in the single repo the client is operating on when it's created (see §Object homing below).

The SDLC vocabulary this hard-coding replaced is gone from the spec (WRIT-194): its prose files, its JSON schemas, its `spec/testdata/` corpora and its `field-rules.json` tables were deleted outright, not deprecated, because nothing had shipped. Its design thinking is preserved outside this repository. What writ keeps is the substrate every vocabulary needed — the envelope, signing, ref layout, canonical encoding, forward compatibility, fold, identifiers, ordering, anchors, resolution, the value-type catalogue, and `schema` itself.

### Schema layer (decided, WRIT-184)

Writ's scope test changes: no longer "does this data explain how the software got made?" — that is a question for whatever is declared above writ — but **"is this git-shaped or merge-shaped?"** Anchors and git object ids are git-shaped and stay in writ's spec, as value types in WRIT-185's closed catalogue. The SDLC vocabulary is not git-shaped — it is ordinary schema-declared data — and it is gone.
...
None of this touches the substrate. `spec/op-envelope.md` already declares `object_type` an open string, not a closed enum, and producer validation rule 3 already reads "the payload satisfies the vocabulary schema for its `object_type`, for every object type the producer itself emits" — already schema-shaped prose. `spec/ref-layout.md` already permits extension chains (`refs/writ/<writer-id>/custom-type`). And `engine/internal/fold` is already a generic driver — `Fold(ops []Op, rules []Rule) → ObjectState{state map[string]any}`, rules keyed by `(op_type, op_version, field)` against a closed strategy catalogue. What was hard-coded was everything layered above that driver: the vocabulary spec prose, most of the JSON schemas and `field-rules.json` tables, the per-type engine facades and projection tables, and the per-type CLI. The follow-up tickets deleted all of it.

### Producer validation is a best-effort pre-flight, not a hard guarantee (decided, WRIT-202)

`Store.vocabularies`' producer-vocabularies cache is fingerprinted over `dag.Chains`, and until WRIT-202, a cache *hit* on the append path still paid one full `dag.Chains` ref walk — every `Append` linear in the repository's total ref count (`BenchmarkAppendByRefCount` at 0/200/500/2,000 loose refs, re-measured for WRIT-202 round 4 on this branch's head — Apple M2 Max, `-benchtime=200x -count=3`, medians of three, "before" being this same tree with the window disabled: 1.06ms → 7.67ms → 17.61ms → 80.30ms per append, against 0.77ms → 0.80ms → 0.94ms → 1.21ms with the window in place, a slope of ~40.1µs per ref falling to ~0.22µs. That ratio is ~180x on this run and has come out anywhere from ~60x to ~200x across runs of the same instrument on the same machine: read it as tens of times, not as a figure to two significant digits. The post-fix column still rises with ref count — the window amortises the walk across a burst, it does not remove it, so the ticket's literal acceptance criterion, per-append cost flat in total ref count rather than linear, is not met and cannot be by this design). The fix that would obviously restore flat per-append cost — trust the rolled-forward snapshot `Store.noteAppend` already maintains, and stop re-deriving from `dag.Chains` at all — reintroduces a correctness bug: a second `writ.Store` handle on the same repository (a CLI plus a watching client, writ's normal case) never fires the first handle's chain observer, so its snapshot goes stale the moment the other handle applies a schema change, with nothing to notice.

Ruled: producer validation is a **best-effort pre-flight, not a hard per-op guarantee**. The spec already implies as much for foreign writers — `spec/schema-source.md`'s "need not parse `writ.schema` at all", and `spec/fold.md` §7.1's reader staying total regardless of what any producer validated — so a producer validating its own ops against a *recently-resolved* view of the log, rather than ground truth on every single op, adds no risk the format did not already carry; `spec/op-envelope.md` §Producer validation states this explicitly. WRIT-202's fix is a 100ms freshness window in front of the append path's `dag.Chains` re-derivation only (`Store.vocabulariesForAppend`): a hit inside the window costs no ref access at all, amortising one scan across a burst of appends instead of paying it per op. `Store.vocabularies` and the readers off it (`rules`, `declaredTypes`, `Refresh`, `Rebuild`, `Types`) are untouched and keep re-deriving on every call — the window binds the append path's own producer pre-flight only. Freshness stays mandatory regardless of the window on a cold cache, immediately after a local `schema` append (`Store.noteAppend` zeroes the cache and its freshness stamp together), and immediately after a `Sync` fetch (`Store.invalidateVocabularies`); the window only ever widens the gap between a *concurrent, external* schema change and this handle's own pre-flight noticing it, never the gap between this handle's own writes and its own pre-flight.

The accepted risk, stated plainly: within the window — strictly, within the window plus at most one ground-truth resolve, because the window is armed when a derive completes and the refs that derive read were read one `Schema()`/`Enumerate` fold earlier — a second handle can sign an op that a schema the first handle just applied would have refused. Those ops are permanent, and what one costs depends on how it violates the schema. An undeclared field is preserved and ignored and the op folds (`spec/forward-compatibility.md`); a value that only contradicts a declared value type also folds, as written, because `spec/fold.md` §7.1 deliberately does not enforce declared value types. Only a value the field's declared merge strategy cannot consume makes the op **uninterpretable**, and there §7.1's blast radius is the whole operation, not the field: rule 1 is "reject the whole operation, not the offending field" — the op contributes no field writes at all, the well-formed fields it also carried included — rule 2 quarantines it through `FC-5`'s opaque-record channel rather than erroring, and rule 3 keeps it in the DAG byte-for-byte. The reader stays total in every case (fold never fails; one bad operation costs that operation, never the object), so the outcome stays visible — but where it lands it costs an operation, not a field, and it does not un-happen. This is accepted, not a defect to close later by widening what the append path observes; the filesystem fast path an earlier round of this same ticket tried, and reverted, is closed for the same reason and is not being reopened by this decision (see the ticket for the two correctness defects that closed it).
...
5. **Projection** — SQLite cache keyed by ref tips: on read, diff tips vs. last fold, fold only deltas, serve queries from indexed tables; still tip-keyed, still droppable/rebuildable, never a source of truth. Its per-type tables are generated from the schema folded out of the log (WRIT-189), not hand-written: one table per declared object type, plus child tables for its collections, alongside six type-agnostic substrate tables (`meta`, `chain_tips`, `code_tips`, `ops`, `objects`, `unknown_ops`). Also home of local-only state: drafts (kept out of shared refs, following Gerrit's draft-handling precedent — publish on intent, never per-keystroke commits), read/unread, sync cursors. An optional DuckDB/Parquet exporter hangs off the same op-log for analytics — derived only, never committed.
...
a projection-shaped workload (5k parent objects, 100k children, one
bulk-insert transaction plus indexed reads by parent) measured cgo at
roughly 1.4–2.2x
