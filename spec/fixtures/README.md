...
- **`op_json_size:`** Pads the `op:` block's `body` with a `pad` string field so the canonicalized `op.json` is exactly this many bytes, for fixtures pinning a byte-length boundary (e.g. reader-validation rule 1's size cap) without checking in megabytes of literal padding — the tree-entry blob SHA in the golden already pins the exact bytes. Valid only alongside an `op:` block, and rejected if smaller than the unpadded payload.
...
- **`expect:`** Declared machine-readable expectation: `accept` or `{reject: <reason>}` from the closed rejection reason set (`non-canonical-payload`, `duplicate-key`, `lone-surrogate`, `schema-violation`, `extra-tree-entry`, `op-json-subdirectory`, `missing-op-json`, `invalid-op-json-mode`, `committer-mismatch`, `payload-too-large`). An accepted commit may also carry `verification: <outcome>` from the closed verification outcome set (`valid`, `wrong-key`, `unsigned`, `corrupted-signature`, `payload-mutated`), defaulting to `valid` when omitted — signature verification never gates fold (WRIT-251), so a bad signature is a verification outcome pinned per commit, not a disposition.
...
- **`trust_store:`** Top-level list of `allowed_signers` rules (`principals`, `key` naming a keyring identity, optional `namespaces`), each becoming one line of the generated `allowed_signers` file (WRIT-302). Present on a description, it *replaces* `AllowedSignersContent`'s generated default (one literal `<identity email> <pubkey>` line per keyring identity) for that description only, so a pattern it adds can't reclassify any other description's commits. Exists because the generated default never emits anything but a bare literal principal, so it can't express `allowed_signers` patterns (globbing, negation, the `namespaces=` option) at all; see `envelope-signer-patterns.yaml` for the shape.
...
- **`schema`:** Golden `writ.FoldSchema` outputs (`testdata/golden/schema/*.json`) driving the one object type writ hard-codes directly, cross-checked asymmetrically against `writ.Fold(ops, writ.SchemaRules())` (spec/schema-ops.md §3.1, §3.3, and §3.4: the typed reducer's non-canonical `op_version` quarantine, its unrepresentable-`max_length` quarantine, and its namespace-implied-by-id quarantine on a `create` all have no equivalent in the generic driver). Covers bootstrap, the §3.1 quarantine, the §3.3 quarantine (`schema-numeric-bounds`), the §3.4 quarantine (`schema-namespace-hijack-quarantined`, WRIT-254), multi-writer concurrent declarations, deprecate/redeclare, and schema-level forward compatibility.
- **`schema-driven`:** Golden outputs (`testdata/golden/schema-driven/*.json`) for every schema object in a fixture folded with `writ.FoldSchema`, resolved into per-`object_type` rules with `writ.RulesFromSchemas`, and every other object folded against the rules resolved for its own `object_type` — with no new engine code, exactly the schema-driven fold path ARCHITECTURE.md §The six machines describes. Covers absent-schema and uninterpretable-op forward compatibility, `spec.ValidateFieldRule` rejection at the §9 boundary, namespace/object-type/redefine-schema collisions and how `RulesFromSchemas` reports them (including the WRIT-254 schema-object-id conflict, `schema-driven-non-derived-id-dropped`, and the rewritten `schema-driven-namespace-collision`/`schema-driven-object-type-collision`, which now pin the drop that conflict causes rather than the collision it made unreachable), `person-ref` normalization across every strategy position that applies it, version-bump target resolution (Correction 3), and reader-side body-vocabulary tolerance under a schema resolved from the log. Every non-schema object is also cross-checked against `spec.Fold`, the normative reference reducer, byte-for-byte on state, `TotalOrder`, and `UnknownOps` (`crossCheckSpecFold`, shared with the `fold` family below) — the same check that surfaced the reffold `person-ref` normalization gap WRIT-274 fixed.
