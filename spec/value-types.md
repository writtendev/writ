...
| `key_types` (object: key column → value type) | Only on `keyed-lww`; must cover exactly the columns `key` declares. A key column's value MUST be a JSON string at producer time regardless of what the entry says, and its *content* is checked against that entry: directly, for a catalogue member already string-encoded, or decoded from text first for `int`/`number`/`bool`/`anchor` — the same string-content encoding `op_version` already uses (`spec/schema-ops.md` §3.1). That content MUST be the value's canonical JSON encoding (`spec/canonicalization.md`) — the decimal string with no leading zero, `+`, trailing fractional zero, exponent, or surrounding whitespace; the object's compact, sorted-member encoding for `anchor` — not merely text that parses to the right shape: fold keys a `keyed-lww` register on the raw string, so two spellings of one value (`"7"` vs `"7.0"`) would otherwise address registers that never converge. `enum` checks no further than the JSON-string floor, since this map has no slot for an enum's member list ([`spec/op-envelope.md`](op-envelope.md) §Producer validation rule 3). Where a key column's name is also a declared field, the field rule governs the value's type, but the JSON-string floor still applies on top of it, and the same content-decode applies to the field's own `value_type` when it is a non-string-shaped member — the field-rule check and the key-column floor are otherwise mutually unsatisfiable for `int`/`number`/`bool`/`anchor`. Two `keyed-lww` rules sharing an `(op_type, op_version)` and a key column name MUST agree on that column's entry here even when their `key` tuples differ. |
