...
4. **Target-agreement failure** (§8): two or more field rules bound to one
   ...
   rules had ever been declared. A field bound to a rule that survives
   folds normally; a field bound to the withheld target has no resolvable
   rule left, which makes it an ordinary unrecognized field on the terms
   `spec/fold.md` §7.1's Scope already sets for every unrecognized field —
   preserved in the log, absent from folded state, and never on its own
   grounds to reject the op that carried it. Withholding a target does
   not, by itself, send an op down the absent-schema path (§7.1): rules
   match an op on `(object_type, op_type, op_version)` alone, with no
   inspection of which fields the op's body actually carries, so a rule
   that survives on the same triple — necessarily one bound to a different
   target — goes on matching the op, whether it writes only the withheld
   target's field, only the surviving target's field, or both in the same
   envelope. Withholding settles nothing further about that op: what else
   can make an op an `UnknownOp` is `spec/fold.md` §7 and §7.1's to say,
