...
func TestFoldCycleEmpty(t *testing.T) {
	state, err := writ.FoldCycle(nil)
	if err != nil {
		t.Fatalf("FoldCycle(nil) returned error: %v", err)
	}
	if !reflect.DeepEqual(state, writ.Cycle{}) {
...
func TestFoldCycleLifecycleAndDates(t *testing.T) {
	...
	state, err := writ.FoldCycle([]codec.Op{createOp, updateOp, setDatesOp})
	if err != nil {
...
func TestFoldCycleMembershipORSetCausalAndConcurrent(t *testing.T) {
	...
	state, err := writ.FoldCycle([]codec.Op{op1, op2, op3})
	if err != nil {
		t.Fatalf("FoldCycle failed: %v", err)
	}
...
func TestFoldCycleCrossRepoReferences(t *testing.T) {
	...
	state, err := writ.FoldCycle([]codec.Op{op1, op2})
	if err != nil {
		t.Fatalf("FoldCycle failed: %v", err)
	}
...
func TestFoldCycleUnknownOpVersionAndType(t *testing.T) {
	...
	state, err := writ.FoldCycle([]codec.Op{v1Create, v2Update})
	if err != nil {
		t.Fatalf("FoldCycle failed: %v", err)
	}
...
func TestFoldCycleMalformedBodyError(t *testing.T) {
	...
	_, errFold := writ.Fold([]codec.Op{badOp}, writ.CycleRules())
	if errFold == nil {
		t.Fatal("expected Fold to error on malformed JSON body, got nil")
	}

	_, errCycle := writ.FoldCycle([]codec.Op{badOp})
	if errCycle == nil {
...
func TestFoldCycleAgreement(t *testing.T) {
	...
	objectState, err := writ.Fold(ops, writ.CycleRules())
	if err != nil {
