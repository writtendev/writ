...
func packfileObjectSize(s storage.Storer, hash plumbing.Hash) (size int64, found bool, err error) {
	...
	return sizeFromDotGit(dotgit.New(fss.Filesystem()), hash, maxAlternateDepth)
}
...
func sizeFromDotGit(dg *dotgit.DotGit, hash plumbing.Hash, altDepth int) (int64, bool, error) {
	if size, found, err := looseObjectSize(dg, hash); found || err != nil {
		return size, found, err
	}

	if size, found, err := packedObjectSize(dg, hash); found || err != nil {
		return size, found, err
	}
	...
	for _, alt := range alternates {
		if size, found, err := sizeFromDotGit(alt, hash, altDepth-1); found || err != nil {
			return size, found, err
		}
	}
...
func packedObjectSize(dg *dotgit.DotGit, hash plumbing.Hash) (int64, bool, error) {
...
func loadPackIndex(dg *dotgit.DotGit, pack plumbing.Hash) (*idxfile.MemoryIndex, error) {
