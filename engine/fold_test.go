...
func TestOrderVectors(t *testing.T) {
	...
	for _, vec := range vectors {
		t.Run(vec.Name, func(t *testing.T) {
			...
			// Test through writ.Fold
			res, err := writ.Fold(ops, []writ.Rule{
				{
					OpType:    "test-op",
					OpVersion: 1,
					Field:     "test-field",
			...
			for i := 0; i < 100; i++ {
				...
				shuffledRes, err := writ.Fold(shuffled, []writ.Rule{
					{
						OpType:    "test-op",
						OpVersion: 1,
						Field:     "test-field",
	...
	for _, vec := range vectors {
		t.Run(vec.Name, func(t *testing.T) {
			...
			for i := 0; i < 100; i++ {
				...
				shuffledRes, err := writ.Fold(shuffled, rules)
				if err != nil {
					t.Fatalf("permutation #%d failed: %v", i, err)
				}
...
func TestFold_EmptyInput(t *testing.T) {
	...
	resEmpty, err := writ.Fold([]codec.Op{}, []writ.Rule{})
	if err != nil {
...
func TestFold_ErrorCases(t *testing.T) {
	t.Run("Cycle", func(t *testing.T) {
		...
		_, err := writ.Fold(cyclicOps, nil)
		if err == nil {
			t.Fatal("expected ErrCycle on cyclic graph, got nil")
		}
		if !errors.Is(err, writ.ErrCycle) {
	...
	t.Run("DuplicateOpID", func(t *testing.T) {
		...
		_, err := writ.Fold(dupOps, nil)
		if err == nil {
			t.Fatal("expected ErrDuplicateOpID on duplicate IDs, got nil")
		}
		if !errors.Is(err, writ.ErrDuplicateOpID) {
	...
	t.Run("MixedObjects", func(t *testing.T) {
		...
		_, err := writ.Fold(mixedOps, nil)
		if err == nil {
			t.Fatal("expected ErrMixedObjects on mixed object IDs, got nil")
		}
		if !errors.Is(err, writ.ErrMixedObjects) {
